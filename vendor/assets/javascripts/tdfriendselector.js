...
var TDFriendSelector = (function(module, $) {
    ...
    newInstance = function(options) {
         ...
         */
        showFriendSelector = function(config, callback) {
            $.extend(instanceSettings['extras'], config);

            var i, len;
            log('TDFriendSelector - newInstance - showFriendSelector');
            if (!$friends) {
                return buildFriendSelector(function() {
                    showFriendSelector(config, callback);
                });
            } else {
                bindEvents();
        ...
        bindEvents = function() {
            ...
            $buttonOK.bind('click', function(e) {
                e.preventDefault();
                hideFriendSelector();
                if (typeof instanceSettings.callbackSubmit === "function") { instanceSettings.callbackSubmit(selectedFriendIds, instanceSettings['extras']); }
            });
